...
function hostSafety(hostname) {
  const host = String(hostname || "").toLowerCase().replace(/^\[|\]$/g, "");
  if (host === "localhost" || host === "::1" || host === "0.0.0.0" || host.endsWith(".localhost") || host.endsWith(".local") || host.endsWith(".internal")) return "loopback";
...
export function classifyFinalUrl(value) {
  ...
  const hostStatus = hostSafety(url.hostname);
  ...
  const sanitized_url = scheme === "http:" || scheme === "https:" ? (hostStatus ? null : cleanUrl.toString()) : null;
