 ...
 *   node scripts/grok_search.mjs "query" --x --days 7 --handles @openai --json
...
const apiKey = resolveKey("XAI_API_KEY").value;
if (!apiKey) {
  console.error(
    "Missing XAI_API_KEY. Run: node scripts/doctor.mjs  (see SETUP.md for generic/host env paths)"
  );
  process.exit(1);
}
